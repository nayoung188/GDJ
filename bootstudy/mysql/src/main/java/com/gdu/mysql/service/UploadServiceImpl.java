...
public class UploadServiceImpl implements UploadService {
   ...
   public void getUploadList(HttpServletRequest request, Model model) {
      ...
      map.put("begin", pageUtil.getBegin() - 1); // mysql은 시작이 0부터임. 1~10
      map.put("recordPerPage", pageUtil.getRecordPerPage());  // begin부터 몇(recordPerPage) 개로 동작함
      
      model.addAttribute("uploadList", uploadMapper.selectUploadList(map));  // -> map 전달 -> Mapper에서 당연히 매개변수로 Map
      model.addAttribute("paging", pageUtil.getPaging("/upload/list")); // 목록의 경로
      // void 처리 했으니 return 없는거여~
   }
