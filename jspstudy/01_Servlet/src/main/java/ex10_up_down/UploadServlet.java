...
public class UploadServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String realPath = getServletContext().getRealPath("upload");		// src/main/webapp(web root 아래 upload)
		...
		String originalName = multipartRequest.getOriginalFileName("filename");		// 원래이름
		...
		String strSize = new DecimalFormat("#,##0").format(size);			// 1000단위 구분 기호의 패턴
		long lastModified = file.lastModified();		// 파일 최종 수정일 (타임스탬프 값으로 나옴)
