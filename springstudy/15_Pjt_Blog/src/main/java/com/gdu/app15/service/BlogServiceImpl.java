...
public class BlogServiceImpl implements BlogService {
	...
	public void getBlogList(Model model) {
		...
		map.put("begin", pageUtil.getBegin());			// 순서 지키기!!계산하고 꺼내올것
		...
		model.addAttribute("paging", pageUtil.getPaging(request.getContextPath() + "/blog/list"));			// 변수처리하면 여려곳에서 사용 가능
	}
	...
	public void saveBlog(HttpServletRequest request, HttpServletResponse response) {
		...
		// 작성자의 ip
		// 작성된 내용이 어딘가를 경유해서 도착하면 원래 ip가 X-Forwarded-For 라는 요청헤더에 저장된다
	...
	public Map<String, Object> saveSummernoteImage(MultipartHttpServletRequest multipartRequest) {
		...
		// 저장할 File 객체
		File file = new File(dir, filesystem);			// dir 대신 path 사용 가능  new File(path, filesystem)
